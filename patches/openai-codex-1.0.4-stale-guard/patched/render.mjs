import { assessJobs, formatStaleWarning } from "./stale-jobs.mjs";
...
export function renderStatusReport(report) {
  ...
  if (report.running.length > 0) {
    ...
    // Surface records that claim to be running but have not been touched in a
    // long time. Reporting only -- see stale-jobs.mjs for why this must never
    // cancel anything on its own.
    const staleWarning = formatStaleWarning(assessJobs(report.running));
    if (staleWarning) {
